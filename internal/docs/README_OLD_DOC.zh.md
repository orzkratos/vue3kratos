...
[demo2-Makefile](https://github.com/orzkratos/vue3kratos-demos/blob/main/demo2kratos/Makefile)
里面有配置样例。

#### 通用配置:
通用的配置是这样的，需要你在 kratos 项目的 Makefile 里面增加这段：
``` makefile
web_api_grpc_ts:
...
### 使用
```
vue3orzkratos gen-grpc-via-http-in-path --grpc-ts-path=/xxx/src/rpc/rpc_admin_login/admin_login.client.ts
```
这个文件里的内容就会被替换，因此特别注意，路径要传正确的绝对路径。

同样的，假如要在 vue 侧转换，请按照例子操作。

这里有个简单的小例子演示: [演示由kratos项目直接得到ts客户端代码](https://github.com/orzkratos/vue3kratos-demos/blob/main/demo1kratos/Makefile)

这里有个简单的小例子演示: [演示由kratos项目直接得到ts客户端代码](https://github.com/orzkratos/vue3kratos-demos/blob/main/demo2kratos/Makefile)
...
工具路径 [工具路径](https://www.npmjs.com/package/@yyle88/grpt)

工具源码 [工具源码](https://github.com/yyle88/grpt)

当然建议还是使用golang侧转换，改完proto和改完接口时运行，即可刷新你的 typescript/javascript-client 代码。
